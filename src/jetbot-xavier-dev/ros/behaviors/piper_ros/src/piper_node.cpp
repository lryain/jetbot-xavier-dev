...
class PiperNode
{
    ...
    piper::Voice m_chineseFemaleVoice;
...
public:
    explicit PiperNode(bool useGpuIfAvailable) : m_useGpuIfAvailable(useGpuIfAvailable)
    {
        ...
        m_englishFemaleVoice = loadVoice("en_US-amy-medium");
        m_englishMaleVoice = loadVoice("en_US-ryan-high");
        
        m_frenchFemaleVoice = loadVoice("fr_FR-siwis-low");
        m_frenchMaleVoice = loadVoice("fr_FR-gilles-low");
        m_chineseFemaleVoice = loadVoice("zh_CN-huayan-medium");
...
private:
    ...
    piper::Voice& voiceFromLanguageAndGender(Language language, Gender gender)
    {
        ...
        else if (language == Language::CHINESE) // 目前只有中文女
        {
            return m_chineseFemaleVoice;
        }
        else
        {
